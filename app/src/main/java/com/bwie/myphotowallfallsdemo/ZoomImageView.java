...
public class ZoomImageView extends View {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (event.getPointerCount() == 1) {
                    ...
                    // 进行边界检查，不允许将图片拖出边界
                    //totalTranslateX  记录图片在矩阵上的横向偏移值
                    //movedDistanceX   记录手指在横坐标方向上的移动距离
    ...
    private void zoom(Canvas canvas) {
        ...
        float translateX = 0f;
        float translateY = 0f;
        //如果当前图片宽度小于屏幕宽度，则按屏幕中心的横坐标进行水平缩放。否则按两指中心点的横坐标进行水平缩放
        if (currentBitmapWidth < width) {
            translateX = (width - scaledWidth) / 2f;
        } else {
            translateX = totalTranslateX * scaledRatio + centerPointX * (1 - scaledRatio);
            //进行边界检查，保证图片缩放后在水平方向不会偏移出屏幕
            if (translateX > 0) {
                translateX = 0;
        ...
        if (currentBitmapHeight < height) {
            ...
            //进行边界检查，保证图片缩放后在垂直方向不会偏移出屏幕
            if (translateY > 0) {
                translateY = 0;
            } else if (height - translateY > scaledHeight) {
                translateY = width - scaledHeight;
            }
        }
        ...
        if (sourceBitmap != null) {
            ...
            if (bitmapWidth > width || bitmapHeight > height) {
                if (bitmapWidth - width > bitmapHeight - height) {
                    ...
                    matrix.postScale(ratio, ratio);//进行缩放，xy的缩放比例一样
