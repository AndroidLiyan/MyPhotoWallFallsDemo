...
/**
 * 类的用途：自定义的ScrollView,在其中动态的对图片进行添加
 *
 * @author 李岩
 * @date ${date} ${time}
 * ${tags}
...
public class MyScrollView extends ScrollView implements View.OnTouchListener {
    ...
    class LoadImageTask extends AsyncTask<String, Void, Bitmap> {
        ...
        private LinearLayout findColumnToAdd(ImageView imageView, int imageHeight) {
            //如果第一列的高度小于等于第二列，然后就继续判断第一列是否小于等于第三列，
            if (firstColumnHeight <= secondColumnHeight) {
                if (firstColumnHeight <= thirdColumnHeight) {
                    //如果第一列小于第二列和第三列就返回第一列的布局
        ...
        private String getImagePath(String imageUrl) {
            int lastSlathIndex = imageUrl.lastIndexOf("/");
            String imageName = imageUrl.substring(lastSlathIndex + 1);
